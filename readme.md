...
2. Install [Docker](https://docs.docker.com/engine/install/ubuntu/) `sudo snap install docker` and [Nvidia Docker](https://github.com/NVIDIA/nvidia-docker#quickstart). And add your current user to the docker group: `sudo groupadd docker && usermod -aG docker $USER`. Than log out and log back.  
